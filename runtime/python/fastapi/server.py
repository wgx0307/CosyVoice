...
async def audio_to_text(
    reference_audio: str = Body(..., description="音频文件路径", example="/path/to/audio.wav"),
    language: Optional[str] = Body("auto", description="识别语言", example="auto"),
    use_itn: Optional[bool] = Body(True, description="是否使用 ITN（逆文本归一化）"),
):
    if not reference_audio:
        return {
            "code": 1,
            "msg": "Missing 'prompt_wav_path' in request body",
            "data": {}
        }
    # todo 需要拼接上路径前缀D:/heygem_data/voice/data
    reference_audio = "D:/heygem_data/voice/data/" + reference_audio
    # 检查文件是否存在
    ...
    try:
        ...
        with open(reference_audio, "rb") as audio_file:
            ...
            # todo: 复制音频文件到指定目录
            audio_url = "test/test.wav"

            return {
                "code": 0,
                "msg": "success",
                "data": {
                    "reference_audio_text": prompt_text,
                    "asr_format_audio_url": audio_url
                }
            }
    except Exception as e:
        return {
            "code": 3,
            "msg": f"Error processing audio: {str(e)}",
            "data": {}
        }
...
async def tts(
        reference_audio: str = Body(..., description="音频文件路径", example="/path/to/audio.wav"),
        reference_text: str = Body(..., description="参考文本", example="音频的文案"),
        text: str = Body(..., description="生成文本", example="测试")
):
    if not reference_audio:
        return {
            "code": 1,
